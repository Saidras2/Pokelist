...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', (e) => {
    if (window.innerWidth <= 768 && sidebar.classList.contains('open')) {
      if (!sidebar.contains(e.target) && !mobileToggle.contains(e.target)) sidebar.classList.remove('open');
    }
  });
...
async function postData(action, payload) {
  ...
  if(!pass) return;
  try {
    const res = await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action, pass, ...payload }) });
    const data = await res.json();
    if(data.success) { await fetchData(); } else { alert("Google Sheets Error: " + data.error); }
...
function getCalculatedData() {
  ...
  let mergedInventory = {};
  let totalValueRp = 0; let totalValueYen = 0;
  ...
  Object.values(mergedInventory).forEach(mergedItem => {
    ...
    if (qtyToAverage === 0) {
      mergedItem._rawItems.forEach(raw => { totalYen += raw.yenPrice; totalBaseRp += raw.basePriceRp; totalShipping += raw.shippingPerCard; totalCost += raw.totalCostPerCard; if(raw.group) activeGroups.add(raw.group); });
      qtyToAverage = mergedItem._rawItems.length || 1;
    } else {
      mergedItem._rawItems.forEach(raw => { if (raw.qty > 0) { totalYen += (raw.yenPrice * raw.qty); totalBaseRp += (raw.basePriceRp * raw.qty); totalShipping += (raw.shippingPerCard * raw.qty); totalCost += (raw.totalCostPerCard * raw.qty); if(raw.group) activeGroups.add(raw.group); } });
    }
...
function renderSalesMetricsChart() {
  ...
  chartPanel.querySelectorAll('.chart-column-node').forEach(node => {
    ...
    node.addEventListener('mouseenter', () => { bubble.style.opacity = '1'; bubble.style.transform = 'translateY(0px)'; if (bar.style.height !== '1%') { bar.style.filter = 'brightness(1.15)'; bar.style.transform = 'scaleY(1.02)'; bar.style.transformOrigin = 'bottom'; } });
    node.addEventListener('mouseleave', () => { bubble.style.opacity = '0'; bubble.style.transform = 'translateY(10px)'; if (bar.style.height !== '1%') { bar.style.filter = 'brightness(1)'; bar.style.transform = 'scaleY(1)'; } });
  });
}
...
if (saveGroupBtn) {
  saveGroupBtn.addEventListener('click', () => {
    ...
    if(name && rate && shipping) {
      ...
      document.getElementById('new-group-name').value = ''; document.getElementById('new-group-rate').value = ''; document.getElementById('new-group-shipping').value = ''; closeModal();
    } else { alert("Please fill out all fields before saving."); }
  });
}
...
function addEmptyCardRow() {
  ...
  tr.innerHTML = `
    ...
    <td data-label="Set Code"><input type="text" class="c-set" placeholder="Set Code"></td>
    <td data-label="Rarity">
      <select class="c-rarity">
        <option value="Promo">Promo</option><option value="C" selected>C</option><option value="U">U</option><option value="S">S</option><option value="RR">RR</option><option value="RRR">RRR</option><option value="AR">AR</option><option value="CHR">CHR</option><option value="SR">SR</option><option value="SAR">SAR</option><option value="UR">UR</option><option value="MUR">MUR</option>
      </select>
    </td>
    <td data-label="Language">
      <select class="c-lang"><option>Japanese</option><option>English</option><option>Indonesian</option></select>
    </td>
  ...
  const applyTranslation = (e) => {
    ...
    let originalText = e.target.value; let translatedText = translatePokemonName(originalText);
    if (originalText !== translatedText) {
      e.target.value = translatedText; e.target.style.transition = 'background-color 0.3s'; e.target.style.backgroundColor = 'rgba(34, 197, 94, 0.2)'; setTimeout(() => e.target.style.backgroundColor = 'transparent', 300);
    }
  };
...
if (saveCardsBtn) {
  saveCardsBtn.addEventListener('click', () => {
    ...
    rows.forEach((row, index) => {
      ...
      if(name) {
        cardsToSave.push({ id: 'CARD_' + Date.now() + '_' + index + '_' + Math.random().toString(36).substr(2, 5), group: group, name: name, set: row.querySelector('.c-set').value, rarity: row.querySelector('.c-rarity').value, language: row.querySelector('.c-lang').value, yenPrice: yen || 0, quantity: row.querySelector('.c-qty').value || 1 });
      }
    });
...
function renderInventory() {
  ...
  if (searchInput) {
    ...
    if (query) {
      filteredList = filteredList.filter(card => {
        const cardName = String(card.name || '').toLowerCase().trim(); const cardSet = String(card.set || '').toLowerCase().trim(); const cardRarity = String(card.rarity || '').toLowerCase().trim();
        if (query.length <= 2) return cardName.startsWith(query); else return cardName.includes(query) || cardSet.includes(query) || cardRarity.includes(query);
      });
    }
  }
  ...
  if (sortVal === 'az') filteredList.sort((a, b) => String(a.name || '').localeCompare(String(b.name || ''))); else if (sortVal === 'za') filteredList.sort((a, b) => String(b.name || '').localeCompare(String(a.name || ''))); else if (sortVal === 'price-high') filteredList.sort((a, b) => Number(b.totalCost || 0) - Number(a.totalCost || 0)); else if (sortVal === 'price-low') filteredList.sort((a, b) => Number(a.totalCost || 0) - Number(b.totalCost || 0));
  ...
  if(filteredList.length === 0) {
    body.innerHTML = `<tr><td colspan="11" style="text-align:center; color: var(--text-secondary);">No matching cards found.</td></tr>`; return;
  }
  ...
  filteredList.forEach(card => {
    const qty = Number(card.quantity || 0); const isOutOfStock = qty <= 0; const tr = document.createElement('tr');
    ...
    tr.innerHTML = `
      <td data-label="Card Name" class="editable-cell edit-name" contenteditable="true" title="Click to edit"><strong style="color:${textColor}">${card.name || '—'}</strong></td>
      <td data-label="Set" class="editable-cell edit-set" contenteditable="true" title="Click to edit">${card.set || '—'}</td>
      <td data-label="Rarity" title="Click to change Rarity">
        <select class="inline-edit-select edit-rarity" style="width: auto; background:transparent; color:inherit; border:1px dashed transparent; outline:none; cursor:pointer; padding:2px 4px; border-radius:4px; font-size:inherit;">
          <option value="">—</option>${rarityOpts.replace(`value="${card.rarity}"`, `value="${card.rarity}" selected`)}
        </select>
      </td>
      <td data-label="Language" title="Click to change Language">
        <select class="inline-edit-select edit-lang" style="width: auto; background:transparent; color:inherit; border:1px dashed transparent; outline:none; cursor:pointer; padding:2px 4px; border-radius:4px; font-size:inherit;">
          <option value="">—</option>${langOpts.replace(`value="${card.language}"`, `value="${card.language}" selected`)}
        </select>
      </td>
      ...
      <td data-label="Base (¥)" class="editable-cell edit-yen" contenteditable="true" title="Click to edit">¥${Number(card.yenprice).toLocaleString('ja-JP')}</td>
    ...
    function saveRowData() {
      ...
      const finalName = nameVal === '—' ? '' : nameVal; const finalSet = setVal === '—' ? '' : setVal; const finalRarity = rarityVal === '—' ? '' : rarityVal; const finalLang = langVal === '—' ? '' : langVal;
      const oldQty = Number(card.quantity) || 0; const isQtyChanged = (qtyVal !== oldQty);
      const isDataChanged = (finalName !== (card.name || '') || finalSet !== (card.set || '') || finalRarity !== (card.rarity || '') || finalLang !== (card.language || '') || yenVal !== card.yenprice);
      ...
      matchingCards.forEach((item, index) => {
        if (isDataChanged) { item.name = finalName; item.set = finalSet; item.rarity = finalRarity; item.language = finalLang; item.yenprice = yenVal; }
        if (isQtyChanged) {
           if (remainingQtyDiff > 0) { if (index === 0) { item.quantity = Number(item.quantity || 0) + remainingQtyDiff; remainingQtyDiff = 0; } } else if (remainingQtyDiff < 0) {
             const currentItemQty = Number(item.quantity || 0); const deductAmt = Math.min(currentItemQty, Math.abs(remainingQtyDiff));
             item.quantity = currentItemQty - deductAmt; remainingQtyDiff += deductAmt;
           }
        }
        silentPostData('updateCard', { id: item.id, name: item.name, set: item.set, rarity: item.rarity, language: item.language, group: item.group, yenPrice: item.yenprice, quantity: item.quantity });
      });
    ...
    tr.querySelector('.btn-delete-card').addEventListener('click', (e) => {
      ...
      if (confirm(`Are you sure you want to delete ${card.name}?`)) {
        ...
        let maxGroupStock = JSON.parse(localStorage.getItem('maxGroupStock')) || {};
        cardsToDelete.forEach(cDel => { const delQty = Number(cDel.quantity || 0); const groupName = cDel.group; if (groupName && maxGroupStock[groupName]) { maxGroupStock[groupName] = Math.max(0, maxGroupStock[groupName] - delQty); } state.inventory = state.inventory.filter(item => item.id !== cDel.id); postData('deleteCard', { id: cDel.id }); });
...
function ensureSaleModalExists() {
  ...
  function handleSearchInput() {
    ...
    if (matches.length === 0) { resultsDiv.innerHTML = '<div style="padding:10px 12px; color:var(--text-secondary); font-size:13px;">No available cards found</div>'; resultsDiv.style.display = 'block'; return; }
...
function renderSaleCart() {
  ...
  saleCart.forEach((item, index) => {
    const itemEl = document.createElement('div'); itemEl.style.cssText = 'padding:14px; border:1px solid var(--border-color); border-radius:8px; background: rgba(128,128,128,0.05); position: relative;';
    const displayPrice = item.price ? 'Rp ' + Number(item.price).toLocaleString('id-ID') : '';
    itemEl.innerHTML = `
      <div style="display:flex; justify-content:space-between; margin-bottom:8px; align-items:center;"><strong style="font-size:14px;">${item.name} <span style="color:var(--text-secondary); font-weight:normal;">(${item.set || 'No Set'})</span></strong><button type="button" class="btn-remove-cart" data-index="${index}" style="background:none; border:none; color:#ef4444; cursor:pointer; padding:4px;" title="Remove"><i class="fas fa-times"></i></button></div>
      <div style="font-size:12px; color:var(--text-secondary); margin-bottom:12px; display:flex; gap:12px;"><span><i class="fas fa-box"></i> Stock: ${item.maxQty} pcs</span><span style="color:var(--accent-yellow);"><i class="fas fa-tag"></i> Base Cost: Rp ${Math.round(item.unitCost).toLocaleString('id-ID')}/pc</span></div>
      <div style="display:flex; gap:12px; flex-wrap:wrap;">
        <div style="flex:1; min-width: 100px;"><label style="display:block; margin-bottom:6px; font-size:12px; color: var(--text-secondary);">Qty Sold</label><input type="number" class="cart-qty" data-index="${index}" value="${item.qty}" min="1" max="${item.maxQty}" style="width:100%; padding:10px; border-radius:6px; border:1px solid var(--border-color); background: transparent; color: inherit; box-sizing: border-box;"></div>
        <div style="flex:2; min-width: 180px;"><label style="display:block; margin-bottom:6px; font-size:12px; color: var(--text-secondary);">Total Selling Price</label><input type="text" class="cart-price" data-index="${index}" value="${displayPrice}" placeholder="e.g. Rp 75.000" style="width:100%; padding:10px; border-radius:6px; border:1px solid var(--border-color); background: transparent; color: inherit; box-sizing: border-box;"></div>
      </div>
    `;
...
function openGlobalSaleSearchModal() {
  ...
  const today = new Date(); const offset = today.getTimezoneOffset() * 60000; document.getElementById('sale-date').value = (new Date(today - offset)).toISOString().split('T')[0]; document.getElementById('modal-sale').style.display = 'block';
}
...
function openSaleModal(card) {
  ...
  selectCardForSale(card, costPerCard); document.getElementById('modal-sale').style.display = 'block';
}

function selectCardForSale(card, costPerCard) { const exists = saleCart.find(c => c.id === card.id); if (!exists) { saleCart.push({ id: card.id, name: card.name, set: card.set, rarity: card.rarity, maxQty: card.quantity, qty: 1, price: '', unitCost: costPerCard }); } renderSaleCart(); }
...
function submitSaleRecord() {
  ...
  const notes = document.getElementById('sale-notes').value; const dateInput = document.getElementById('sale-date').value; const finalSaleDate = dateInput ? new Date(dateInput + 'T00:00:00').toISOString() : new Date().toISOString();
  ...
  saleCart.forEach(cartItem => {
    ...
    matchingCards.forEach(targetCard => {
      ...
      state.sales.push({ date: finalSaleDate, cardid: targetCard.id, name: targetCard.name, set: targetCard.set, rarity: targetCard.rarity, quantity: qtyToDeductFromThisCard, price: splitSalePrice, notes: notes });
      silentPostData('recordSale', { date: finalSaleDate, cardId: targetCard.id, name: targetCard.name, set: targetCard.set, rarity: targetCard.rarity, quantity: qtyToDeductFromThisCard, price: splitSalePrice, notes: notes });
  ...
  state.groups.forEach(g => {
    ...
    div.querySelector('.btn-delete-group').addEventListener('click', () => {
      ...
      if(confirm(confirmMsg)) {
        ...
        state.groups = state.groups.filter(item => item.id !== g.id); state.inventory = state.inventory.filter(item => item.group !== g.name); updateDashboard(); renderInventory(); renderGroups(); renderTrash(); postData('deleteGroup', { id: g.id, name: g.name });
      }
    });
...
function renderSales() {
  ...
  filteredSales.forEach((sale, index) => {
    ...
    tr.innerHTML = `
      ...
      <td data-label="Notes" class="editable-cell edit-sale-notes" contenteditable="true" title="Click to edit">${sale.notes || '—'}</td>
      <td data-label="Action">
        <div style="display: flex; gap: 4px; align-items: center; justify-content: flex-end;">
    ...
    tr.querySelector('.btn-delete-sale').addEventListener('click', () => { if (confirm(`Are you sure you want to delete this sale record for ${sale.name}? This will restore ${sale.quantity} stock back to the inventory.`)) { if (linkedCard) linkedCard.quantity = Number(linkedCard.quantity || 0) + Number(sale.quantity || 1); state.sales = state.sales.filter(s => s !== sale); updateDashboard(); renderSales(); renderInventory(); postData('deleteSale', { date: sale.date, cardId: sale.cardid, quantity: sale.quantity }); } });
    body.appendChild(tr);
  });
  ...
  if (checkAll && itemChecks && bulkDeleteBtn && bulkCountText && bulkDiv) {
    ...
    newBulkBtn.addEventListener('click', () => {
      ...
      if (confirm(`Are you sure you want to delete ${checkedBoxes.length} sale record(s)? This will restore their stock back to the inventory.`)) {
        ...
        (async function syncBulkDelete() { for (const sale of salesToDelete) { try { await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'deleteSale', date: sale.date, cardId: sale.cardid, quantity: sale.quantity }) }); } catch(e) {} } })();
      }
    });
  }
}
...
function renderTrash() {
  ...
  sortedTrash.forEach((item, index) => {
    ...
    const typeBg = item.type === 'Card' ? 'rgba(59, 130, 246, 0.15)' : item.type === 'Sale' ? 'rgba(34, 197, 94, 0.15)' : 'rgba(168, 85, 247, 0.15)'; const typeColor = item.type === 'Card' ? '#3b82f6' : item.type === 'Sale' ? '#22c55e' : '#a855f7'; const displayType = item.type === 'GroupBundle' ? 'Group & Cards' : item.type;
  ...
  if (checkAll && itemChecks && bulkRestoreBtn && bulkDeleteBtn && bulkCountText && bulkDiv) {
    ...
    newBulkRestore.addEventListener('click', () => {
      ...
      if (confirm(`Are you sure you want to restore ${checkedBoxes.length} item(s)?`)) {
        const itemsToRestore = Array.from(checkedBoxes).map(chk => sortedTrash[chk.getAttribute('data-index')]); itemsToRestore.forEach(item => processRestoreState(item)); updateDashboard(); renderInventory(); renderGroups(); renderTrash();
        (async function syncBulkRestore() { for (const item of itemsToRestore) { try { await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'restoreTrash', trashId: item.id }) }); } catch(e) {} } })();
      }
    });
    ...
    newBulkDelete.addEventListener('click', () => {
      ...
      if (confirm(`Permanently delete ${checkedBoxes.length} item(s)? This cannot be undone.`)) {
        const itemsToDelete = Array.from(checkedBoxes).map(chk => sortedTrash[chk.getAttribute('data-index')]); itemsToDelete.forEach(item => { state.trash = state.trash.filter(t => t.id !== item.id); }); renderTrash();
        (async function syncBulkDelete() { for (const item of itemsToDelete) { try { await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'deleteTrash', trashId: item.id }) }); } catch(e) {} } })();
      }
    });
  }
}
...
function restoreSingleTrash(item) { processRestoreState(item); updateDashboard(); renderInventory(); renderGroups(); renderTrash(); postData('restoreTrash', { trashId: item.id }); }
